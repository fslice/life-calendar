...
const monoFont500 = readFileSync(join(__dirname, "..", "fonts", "ibm-plex-mono-500.woff2"));
const monoFont400 = readFileSync(join(__dirname, "..", "fonts", "ibm-plex-mono-400.woff2"));
const sansFont400 = readFileSync(join(__dirname, "..", "fonts", "inter-400.woff2"));
...
export default function handler(req, res) {
  ...
  ${rects}
  <text x="${W / 2}" y="${bottomY}" text-anchor="middle" font-family="IBM Plex Mono" font-size="80" font-weight="500" fill="#1a1a1a">${remaining}</text>
  <text x="${W / 2}" y="${bottomY + 55}" text-anchor="middle" font-family="Inter" font-size="36" font-weight="400" fill="#999999">days to go</text>
  <text x="${W / 2}" y="${bottomY + 110}" text-anchor="middle" font-family="IBM Plex Mono" font-size="32" font-weight="400" fill="#bbbbbb">${pct}%</text>
  <text x="${W / 2}" y="${bottomY + 155}" text-anchor="middle" font-family="Inter" font-size="26" font-weight="400" fill="#cccccc">${dateStr}</text>
</svg>`;
  ...
  const resvg = new Resvg(svg, {
    fitTo: { mode: "original" },
    font: {
      fontBuffers: [monoFont500, monoFont400, sansFont400],
